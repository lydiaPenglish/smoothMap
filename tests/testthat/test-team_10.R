...
test_that("Function team_10 doesn't work.", {
  library(dplyr)
  ...
  expect_error(team_10(file2, tolerance = 0.01)) # not a shape file
  expect_error(team_10(file, tolerance = "A")) # tolerance is not a number
  expect_error(team_10(file, tolerance = -0.01)) #tolerance is negative
  expect_warning(team_10(file, tolerance = 2)) # tolerance is very high
